  ...
  override def run(sourceContext: SourceFunction.SourceContext[StationLog]): Unit = {
    ...
    while (isStop) {
      1.to(10).map(_ => {
        ...
        //生成一条数据 可能会有1分钟的延迟
        val log = StationLog("station_" + r.nextInt(10), callOut, callIn, types(r.nextInt(4)), System.currentTimeMillis() - Random.nextInt(1000 * 60 * 100) + Random.nextInt(1000 * 10 * 100), r.nextInt(20))
        log
